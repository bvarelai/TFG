...
def create_inscription(db: Session, inscription: InscriptionCreate):
    db_inscription = Inscription(event_id = inscription.event_id, user_id = inscription.user_id, event_name = inscription.event_name ,inscription_date=datetime.now().replace(microsecond=0),start_date=inscription.start_date,end_date=inscription.end_date,location=inscription.location)
